 ...
 * UInt32
 *
 * Represents a UInt32 for use by the evaluator within C Memory
...
/**
 * Allows viewing of UInt32 explicit_control_evaluator views including for the C Heap
 *
...
export default class UInt32 {
    ...
    /**
     * Initialises a new C UInt32 viewer without the ability to overwrite protected values and in big-endian format
     * @param view The explicit_control_evaluator view with the UInt32
     */
    public constructor(view: DataView);

    /**
     * Initialises a new C UInt32 viewer with the ability to overwrite protected values and in big-endian format
     * @param view The view of the heap with the UInt32
    ...
    /**
     * Initialises a new C UInt32 viewer with the ability to overwrite protected values
     * @param view The explicit_control_evaluator view with the UInt32
    ...
    public get value(): number {
        if (this.data instanceof HeapDataView) {
            return this.data.get_value(0, UInt32.byte_length).getUint32(0, this.little_endian);
        }
    ...
    public set value(new_value: number) {
        ...
        // Get value
        const value = new ArrayBuffer(UInt32.byte_length);
    ...
    public static create_buffer(value: number): ArrayBuffer {
        const value_buffer = new ArrayBuffer(UInt32.byte_length);
        new UInt32(new DataView(value_buffer)).value = value;
        return value_buffer;
    }
}